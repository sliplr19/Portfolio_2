...
Now we're ready to do some networks! We're going to do separate networks for schizophrenia and schizoffective, so we'll start with the schizophrenia EGA. I ran the code once to see what the structure is. I'll also add the legend at this step based on the previous run.
...
SZ_plot <- plot(SZ_EGA, plot.args = list(vsize = 5, alpha = 0.5, edge.alpha = 0.5,  legend.names = c("Factor 1", "Factor 2", "Factor 3", "Factor 4", "Factor 5", "Factor 6", "Factor 7"),  color.palette = "rainbow", label.size = 3))
...
SZ_data <- SSD_data %>%
  filter(SSD == "Schizophrenia") 
SZ_corr <-  polycor::hetcor(SZ_data[,1:50])